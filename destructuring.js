...
var totalPopulation = (obj) => {
  let states = {utah, california, texas, arizona} = obj;
  let statesArr = [utah, california, texas, arizona];
  return statesArr.reduce(function(a,b){
    return a + b;
  })
}
...
function numberGroups( obj ) {
  ...
  for(let i=0;i<groupArr.length;i++){
    if(groupArr[i].length > longest.length){
      longest = [];
      longest.push(...groupArr[i]);
    }
  }
  return longest;
}